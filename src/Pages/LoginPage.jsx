...
<div className="container-sm mt-5  text-center">
 ...
 <h1 >{estadoRegistrados ? 'Registrate' : "Iniciar Sesión" }</h1>
  ...
  <div className="mb-3">
    ...
    <input type="email" className="form-control" id="formBasicEmail" aria-describedby="emailHelp" />
  ...
  <button type="submit" className="btn btn-dark">{estadoRegistrados ? 'Registrate' : "Iniciar sesión" }</button>

  <div className=' mt-3'>
  <button type="submit" className="btn btn-primary m-3" style={{width:'300px'}} onClick={()=>signInWithRedirect(auth,googleProvider)}>Acceder con google</button>

  <button type="submit" className="btn btn-dark" style={{width:'400px'}} onClick={()=>setRegistrados(!estadoRegistrados)}>
      {estadoRegistrados ? '¿Ya tienes cuenta? Inicia sesión': '¿No tienes cuenta? Registrate'}</button>
